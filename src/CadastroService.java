...
public class CadastroService extends JFrame implements ActionListener {
    ...
    public void cadastrarEntidade(CadastroSCP scp) {
        String verificarSql = "SELECT COUNT(*) FROM TB_CAOS_SCP WHERE nm_nomenclatura = ?";
        String inserirSql = "INSERT INTO TB_CAOS_SCP (nm_nomenclatura, tp_scp_sexo, nm_apelido, tp_classe, ds_descricao, tp_contencao) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public void alterarEntidade(CadastroSCP scp){
        String alterarSql = "UPDATE TB_CAOS_SCP SET nm_nomenclatura = ?, tp_scp_sexo = ?, nm_apelido = ?, tp_classe = ?, ds_contencao = ?, tp_contencao = ? WHERE nm_nomenclatura = ?";
